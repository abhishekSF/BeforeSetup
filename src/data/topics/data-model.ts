...
export const dataModelTopics: Topic[] = [
  {
    ...
    mentalModel: [
      ...
      "The concept that saves hours is the external ID. Mark a field like ERP_Number__c as an external ID and you can upsert. That is update-or-insert in one operation. You can also set lookups by the external value instead of hunting for Salesforce IDs. A load spec that maps every column, states the order, and names the external IDs is the difference between a clean migration and a week of cleanup.",
    ],
    whenToUse: [
    ...
    whenToAvoid: [
      "Ongoing high-frequency sync is an integration. Use APIs, events, or middleware, not a nightly CSV ritual.",
  ...
  },
  {
    ...
    mentalModel: [
      ...
      "Dashboards are collections of report-backed components that display data as a specific running user. Viewers may see more or less than their own access would allow. That is a feature for executive rollups and a security consideration at the same time. Dynamic dashboards, which view as the logged-in viewer, close that gap. For analytics beyond native reports, such as cross-object math at scale or external data, the ladder continues into CRM Analytics and Tableau, now tied together by Tableau Semantics from the Data 360 side.",
    ],
    whenToUse: [
      ...
      "Report charts embedded on record pages via App Builder. Analytics sit where the work happens.",
    ],
    whenToAvoid: [
      "Deep multi-source analytics, statistical work, or years of history belong in Tableau, CRM Analytics, or a warehouse, not a 2,000-row native report grid.",
      "Using reports as an integration, as in 'we export this CSV nightly and load it into...', is an API job wearing a report costume.",
    ],
    pitfalls: [
  ...
  },
  {
    ...
    mentalModel: [
      ...
      "The important shift from spreadsheet thinking is that every object automatically gets an ID, ownership, audit fields, a security model, page layouts, reporting, and an API. Creating an object also creates a full mini-application. That is why data model decisions ripple into everything else: automation, security, UI, and integrations all hang off objects and fields.",
    ],
    whenToUse: [
    ...
    whenToAvoid: [
      ...
      "Don't model transactional, high-volume machine data, millions of rows per day, as regular custom objects. Look at Big Objects, Platform Events, or keeping it off-platform.",
      "Avoid duplicating a standard object, such as a custom 'Company__c' next to Account. You lose all the standard functionality wired to the original.",
    ],
    pitfalls: [
      ...
      "Renaming labels doesn't rename API names. Reports and code reference API names, so a field labeled 'Region' may really be 'District__c' underneath. That stays confusing forever.",
  ...
  },
  {
    ...
    packaging: "core",
    tagline:
      "How records point at each other. The choice between lookup and master-detail decides deletion, security, and rollups.",
    mentalModel: [
      "A relationship is a field on one object that holds the ID of a record on another object. It is a foreign key with a UI. Salesforce gives you two main flavors. A lookup is a loose reference: the child can exist without the parent, and each record keeps its own owner and sharing. A master-detail is a tight bond: the child's lifecycle, ownership, and security are controlled by the parent, deleting the parent deletes the children, and you unlock roll-up summary fields (sum/count/min/max of children on the parent).",
      "The practical question to ask is whether this record can exist meaningfully on its own. An Invoice Line without an Invoice is meaningless, so use master-detail. A Contact's preferred Hotel is just a reference, so use a lookup. For many-to-many, such as Students and Courses, you create a junction object: a small object with two master-detail fields, one to each side.",
    ],
    whenToUse: [
    ...
    pitfalls: [
      ...
      "Self-relationships, such as Account parent Account, are fine, but hierarchy roll-ups across them need code or tooling. There is no native multi-level roll-up.",
    ],
  ...
  },
  {
    ...
    mentalModel: [
      "Sometimes one object needs to serve two processes. A Case might be a support ticket or an internal IT request. An Opportunity might follow a new-business or a renewal process. Record types are a field on the record that switches which picklist values, page layouts, and, for some objects, business processes apply.",
    ...
    whenToAvoid: [
      "Don't use record types purely to show or hide a few fields. Dynamic Forms with field visibility rules is lighter.",
    ...
    pitfalls: [
      ...
      "Automation that forgets to filter by record type fires for flavors it was never meant for. That is a classic source of 'why did this email go out?'",
