...
export const automationTopics: Topic[] = [
  {
    ...
    packaging: "core",
    tagline:
      "Formulas that block bad data at save time. They are the cheapest data quality tool on the platform, and the easiest to overdo.",
    mentalModel: [
      "A validation rule is a formula that evaluates on save. If it returns true, the save is blocked and the user sees your error message. That's the whole mechanism, as in 'Close Date can't be in the past', 'Amount is required once Stage is Negotiation', or 'Discount above 30% needs a reason'. They run on every save from every source. UI, API, data loads, flows, and Apex all hit the same wall, which is exactly what makes them trustworthy as a data-quality floor.",
      "The design skill is deciding what belongs in a validation rule versus elsewhere. Rules can only block and message. They can't fix, default, or transform. That work belongs in Flow or triggers. And because they fire on every save path, a rule written with only the UI in mind, as in 'the user should just fill this in!', becomes a landmine for integrations and migrations that legitimately save records in intermediate states.",
    ],
    whenToUse: [
      ...
      "Enforcing rules that must hold no matter who or what saves the record, whether human, flow, or integration.",
    ...
    whenToAvoid: [
      ...
      "Cross-record validation at scale, such as checking siblings or children. Formula rules can't query. That is trigger territory, with limits in mind.",
    ],
    pitfalls: [
      ...
      "Vague error messages such as 'Invalid data!' that generate tickets instead of preventing them. Say what's wrong and how to fix it, and place the error on the field.",
  ...
  },
  {
    ...
    packaging: "core",
    tagline:
      "Salesforce's declarative automation engine. It is the default answer to 'make something happen automatically' without code.",
    mentalModel: [
      "Flow is a visual programming language. You drag elements onto a canvas: get records, update records, if/else decisions, loops, and screens for user input. Salesforce runs them. A record-triggered flow fires when a record is created, updated, or deleted. A screen flow walks a user through a guided form. A scheduled flow runs on a timer. An autolaunched flow is a subroutine other automation can call.",
    ...
    whenToUse: [
      "Field updates, record creation, notifications, and approvals driven by record changes. This is the core of org automation.",
    ...
    whenToAvoid: [
      "Complex logic with heavy branching, recursion, or nested data structures. Apex is easier to test, review, and debug past a certain complexity.",
      ...
      "Logic that must be unit tested with rigor, such as revenue calculations or compliance rules. Flow tests exist but Apex testing is far more mature.",
    ],
    pitfalls: [
  ...
  },
  {
    ...
    mentalModel: [
      ...
      "The golden rule: triggers receive records in batches, up to 200 at a time. Trigger.new is always a list. Every trigger must be written bulkified, with no queries or DML inside loops, because a data load will hand it 200 records at once. The standard architecture is one trigger per object that delegates to a handler class, keeping logic testable and ordering explicit.",
    ],
    whenToUse: [
    ...
    whenToAvoid: [
      ...
      "Callouts to external systems. Triggers can't make synchronous callouts. You'd need async via Queueable anyway, so consider Platform Events or Flow.",
    ],
    pitfalls: [
  ...
  },
  {
    ...
    packaging: "core",
    tagline:
      "Structured sign-off chains. They define who must approve a record, in what order, and what happens at each step.",
    mentalModel: [
      "An approval process is a state machine for sign-offs. A record is submitted, manually or by automation. Salesforce locks it, routes it to one or more approvers in sequence or parallel, and runs actions at each stage, on submit, approve, reject, or recall. Typical uses are discount approvals, PTO requests, and contract sign-offs.",
      "The key mental shift is that approval is a process attached to the record, not a field. The record lock during approval is real. Users and most automation can't edit a locked record, which protects integrity mid-review but surprises teams that expect to keep working on it. Flow has increasingly absorbed approval use cases via Approval Orchestration, but classic approval processes remain the workhorse.",
    ],
    whenToUse: [
    ...
    whenToAvoid: [
      ...
      "Complex dynamic routing, such as approver matrices from external data. Classic approvals get awkward. Consider Flow Orchestration or an AppExchange tool.",
    ],
    pitfalls: [
      ...
      "Recall and reject paths are afterthoughts in most builds. Decide what should reset, including stage, fields, and locks, when someone rejects.",
  ...
  },
  {
    ...
    mentalModel: [
      "Synchronous Apex must finish inside one user transaction with strict limits. Asynchronous Apex says queue this work, run it soon, in a fresh transaction with higher limits. There are four flavors. Queueable is the modern default, one job that can chain to another. Batch Apex processes millions of records in chunks of up to 200. Scheduled Apex runs at a time, like cron. @future is legacy fire-and-forget. Prefer Queueable now.",
    ...
    whenToUse: [
      ...
      "Nightly or weekly jobs such as cleanup, recalculation, or sync. Scheduled Apex kicking off a Batch or Queueable.",
