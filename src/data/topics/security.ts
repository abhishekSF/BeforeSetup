...
export const securityTopics: Topic[] = [
  {
    ...
    mentalModel: [
      "Salesforce identity has two directions. Inbound (people logging into Salesforce): My Domain gives your org its own login URL, SSO delegates authentication to your identity provider (Okta, Entra ID) over SAML or OpenID Connect, and MFA is mandatory. SSO delegates sign-in; it does not automatically remove Salesforce credentials or disable direct login. My Domain settings, authentication policies, and user configuration decide the available login routes. Outbound and app-to-app: Connected Apps (and the newer External Client Apps) are the OAuth registrations that let integrations, mobile apps, and now MCP clients get tokens to call your org's APIs.",
      "The mental model that simplifies everything: authentication answers 'who are you?' once, at the front door; everything you've learned about profiles, permission sets, and sharing answers 'what can you do?' after that. SSO doesn't change your security model — it changes who verifies the human. Provisioning and deprovisioning are a separate identity-lifecycle job. Configure and test SCIM or the provider’s supported user-management integration; SSO alone does not deactivate a Salesforce user or revoke existing sessions and tokens. Include those in the offboarding runbook.",
    ],
    whenToUse: [
      ...
      "JIT can create or update users at login. For offboarding, use a supported provisioning connector or SCIM integration and explicitly test deactivation and token/session revocation.",
    ],
    whenToAvoid: [
  ...
  },
  {
    ...
    pitfalls: [
      "Muting is local to a Permission Set Group. A permission granted elsewhere still applies; inspect all grant sources before assuming a mute removes access.",
      ...
      "Check Apex class API versions and explicit execution modes, including the API 67.0 boundary. Legacy system-mode code and triggers need particular care; page visibility is never a substitute for server-side authorization.",
