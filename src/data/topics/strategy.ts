...
export const strategyTopics: Topic[] = [
  ...
  },
  {
    ...
    mentalModel: [
      ...
      "The toolkit maps to the problems: custom indexes and external IDs for selectivity, skinny tables for hot reporting queries (via Support), distributing ownership and parenting to avoid skew, Big Objects or off-platform archives for cold data, and PK chunking for extracting huge tables. The strategic mindset is to design for volume before you have it. Retrofitting selectivity onto a 50-million-row object is a project. Designing it in was a meeting.",
    ],
    whenToUse: [
      "Any object projected past ~1 to 2 million rows: decide indexes, ownership distribution, and archive policy up front.",
  ...
  },
  {
    ...
    mentalModel: [
      "Two axes decide what's possible before any design conversation. The edition (Starter, Professional, Enterprise, Unlimited) sets org-level capabilities: API access, number of sandboxes, automation limits, advanced features. Enterprise is the de facto business standard. The user license (Salesforce, Salesforce Platform, Experience Cloud licenses, and a zoo of add-ons) sets what each human can access: full CRM objects versus platform-only (custom objects plus a subset of standard ones).",
      "The architectural insight consultants internalize: licensing is a design input, not a procurement afterthought. Platform licenses cost a fraction of full licenses, so an internal app for 500 warehouse staff might be designed around custom objects specifically so it works on Platform licenses. Experience Cloud login-based versus member-based pricing decides portal architecture. AI features are consumption-priced (credits/usage), which means budgeting shifts from 'per seat per month' to 'per unit of work'. That's a different conversation with finance entirely.",
    ],
    whenToUse: [
    ...
    pitfalls: [
      ...
      "Sandbox entitlements vary by edition. A deployment strategy that assumes a Full Copy sandbox the customer doesn't have will fail.",
  ...
  },
  {
    ...
    mentalModel: [
      "Salesforce projects rarely fail on technology. They fail on requirements and adoption. The lifecycle that works starts with discovery. Understand the process as it actually runs, not as the org chart claims. Shadow the users. Design the data model and security model first, screens later, and write decisions down. Build in a sandbox, in vertical slices users can react to, not horizontal layers. UAT uses real users, real scenarios, and real data shapes, not the consultant clicking through happy paths. Go-live needs a cutover runbook and a rollback plan. Hypercare is the two weeks after launch when trust is won or lost.",
      "Two consultant instincts are worth developing early. Requirements are hypotheses, not specs. 'The client asked for it' is not a reason to build something that won't get used. Every customization has a maintenance tail. The elegant thing you build today is the thing an admin must understand in three years. Bias toward standard features, document the deltas, and leave the org more understandable than you found it.",
    ],
    whenToUse: [
