...
export const codeTopics: Topic[] = [
  ...
  },
  {
    ...
    mentalModel: [
      "Production Apex deployments normally require passing tests and coverage. RunLocalTests excludes installed managed-package tests; RunAllTestsInOrg includes them. RunSpecifiedTests applies coverage requirements to deployed classes and triggers individually. Eligible validated deployments can use Quick Deploy without rerunning tests; deployments without Apex can have different requirements. Check the deployment test level, not just a blanket 'all tests run' rule. Tests are Apex methods marked @isTest that create data, run your code, and assert outcomes. Tests see no org data by default (and shouldn't — @isTest(SeeAllData=true) is a legacy trap), so each test builds the world it needs.",
