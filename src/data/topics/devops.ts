...
export const devopsTopics: Topic[] = [
  {
    ...
    pitfalls: [
      "CMDT records can't be created with regular DML in Apex (they deploy, or go through the special metadata API path). That's a design surprise if you expected runtime writes.",
  ...
  },
  {
    ...
    mentalModel: [
      "A sandbox is a copy of your production org's configuration (and sometimes data) where you build and test without risk. They come in sizes: Developer (config only, small data allowance), Developer Pro (more storage), Partial Copy (config plus a sample of data), and Full Copy (everything, refreshed rarely because it's expensive and slow). Scratch orgs are different. They are blank, short-lived orgs created from a config file in seconds, designed for source-driven development.",
    ...
    pitfalls: [
      ...
      "Email deliverability defaults protect you (sandboxes don't send real email by default). Automation, scheduled jobs, and integrations may still fire. Deactivate them on refresh.",
  ...
  },
  {
    ...
    mentalModel: [
      "A deployment moves metadata from one org to another. The tools form a maturity ladder. Change sets let you click-to-select changes in a sandbox and upload to production. They are simple, but manual, unversioned, and easy to under-scope. DevOps Center is Salesforce's free git-backed release tool. It covers work items, review, and promotion between environments with version control underneath, so admins and developers share one pipeline. Full CI/CD runs the CLI in GitHub Actions or similar, validating every pull request against a sandbox and deploying on merge.",
      "Whatever the tool, production deployments run your Apex tests and enforce 75% coverage. That's the platform's gate. The discipline that matters more than tooling is this. Production is read-only for humans. Everything moves through the pipeline. Every release has a rollback plan. Salesforce has no native rollback, so that plan means knowing how to deploy the previous state.",
    ],
    whenToUse: [
      "DevOps Center as the default for most teams. It's free, git-backed, and a huge upgrade from change sets without needing a DevOps engineer.",
