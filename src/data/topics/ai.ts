...
export const aiTopics: Topic[] = [
  {
    ...
    mentalModel: [
      "Agentforce is Salesforce's system for building AI agents that work on your CRM data: you define an agent with topics (what it's allowed to handle) and actions (what it can actually do — flows, Apex, prompt templates), and the Atlas Reasoning Engine runs the plan-and-act loop, deciding which action fits the request. The critical design fact: agents run under Salesforce's existing security model. Start with the execution identity: employee-facing agents can act in the logged-in employee’s context; customer-facing service agents can use a dedicated Agent User. That user is not the customer chatting with the agent. Audit its permissions, customer verification, and the execution context of each Flow or Apex action — never assume the chat participant’s access automatically constrains every action.",
      "You also need the naming decoder ring, because at Dreamforce 2025 Salesforce put nearly everything under an 'Agentforce 360' umbrella: Sales Cloud became Agentforce Sales, Service Cloud became Agentforce Service, Data Cloud became Data 360, and Einstein 1 Platform positioning became Agentforce 360 (comprising the Agentforce 360 platform, Data 360, the Customer 360 apps, and Slack). Product lineage can continue through a rename, but packaging, entitlements, and pricing can change. Match the capability as well as the name, and check your contract. When a tutorial says 'Sales Cloud' and a sales deck says 'Agentforce Sales', they mean the same thing.",
    ],
    whenToUse: [
    ...
    pitfalls: [
      ...
      "Pricing surprises: consumption and user-based options coexist. Measure real pilot usage and verify what your agreement includes before org-wide rollout.",
  ...
  },
  {
    ...
    tagline:
      "Salesforce's unified data layer — for connecting, harmonizing, and grounding experiences in data from multiple systems.",
    mentalModel: [
      ...
      "The strategic shift to understand: Data 360 stopped being a marketing-adjacent CDP and became the grounding layer for Agentforce. Intelligent Context grounds agents in unstructured data (documents, transcripts), and Tableau Semantics lets you define metrics once and reuse them across analytics and agents. Salesforce's own positioning is blunt — agents without unified, governed data produce unreliable output — but the data architecture should follow the use case: not every agent requires a separate data-unification project. Salesforce announced its definitive agreement to acquire Informatica on May 27, 2025, and completed the acquisition on November 18, 2025. Completion is not a promise that every integration or entitlement is already available in your org.",
    ],
    whenToUse: [
      "When agents need governed context across fragmented systems. Start with the retrieval problem; do not commission a data-unification project just because an agent is in the design.",
