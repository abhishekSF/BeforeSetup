...
export const aiTopics: Topic[] = [
  {
    ...
    mentalModel: [
      "Agentforce is Salesforce's system for building AI agents that work on your CRM data. You define an agent with topics (what it's allowed to handle) and actions (what it can actually do, such as flows, Apex, and prompt templates). The Atlas Reasoning Engine runs the plan-and-act loop, deciding which action fits the request. The critical design fact is that agents run under Salesforce's existing security model. Sharing rules, field-level security, and permissions constrain an agent exactly like a human user. The governance you already built is the guardrail.",
    ...
    pitfalls: [
      ...
      "Vague topic and instruction design. That's the agent equivalent of a bad prompt. Tight topic scoping and testable instructions separate demos from production.",
  ...
  },
  {
    ...
    mentalModel: [
      "Data 360 ingests data from everywhere (CRM, marketing, commerce, warehouses, external systems), harmonizes it into a standard model, resolves identities ('these five records are the same person'), and produces unified profiles that every cloud and every agent can use. Zero-copy connections to Snowflake and Databricks mean it can reference warehouse data without duplicating it. If you've heard of Customer 360 Audiences, Salesforce CDP, Marketing Cloud CDP, Genie, or Data Cloud, those are the same product line. Data 360 is the sixth name, applied at Dreamforce 2025.",
      "The strategic shift to understand: Data 360 stopped being a marketing-adjacent CDP and became the grounding layer for Agentforce. Intelligent Context grounds agents in unstructured data (documents, transcripts), and Tableau Semantics lets you define metrics once and reuse them across analytics and agents. Salesforce's own positioning is blunt. Agents without unified, governed data produce unreliable output. That's why 'implement Data 360 first' is now the standard Agentforce prerequisite. The pending Informatica acquisition (announced late 2025) is pushing governance and data quality deeper into the platform.",
    ],
    whenToUse: [
  ...
  },
  {
    ...
    whenToUse: [
      "Connecting AI clients such as Claude, Cursor, ChatGPT, and Postman to your org for governed data access and actions without custom integration code.",
      "Building agent experiences that must render across surfaces (Slack, Teams, web) from one implementation. That's the Headless Experience Layer's job.",
    ...
    pitfalls: [
      ...
      "Note the two different auth postures in the ecosystem: Hosted MCP Servers run per-user OAuth, but Slack's Claude Tag route uses a client-credentials integration user. That's one identity for everyone in scope. Know which one you're enabling.",
    ],
  ...
  },
  {
    ...
    pitfalls: [
      ...
      "Don't run the pilot through the same sandbox window as your Winter '27 upgrade (production waves September to October 2026). Two simultaneous changes make failures unattributable.",
