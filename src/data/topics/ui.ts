...
export const uiTopics: Topic[] = [
  {
    ...
    mentalModel: [
      "Experience Cloud (formerly Communities) builds websites and portals on top of your org: a customer support portal where users file and track their own cases, a partner portal where resellers manage their deals, a public knowledge site. The pitch is that external users work directly against the same objects your internal teams use, with no sync layer. Templates and the Experience Builder handle the site-building.",
      "The mental model that prevents incidents: external users are a different species in the security model. They get portal-specific licenses, they sit below the role hierarchy, and org-wide defaults have a separate 'external' column. Guest (unauthenticated) users are locked down harder still, and sharing must be opened deliberately per object. Internal assumptions ('everyone can see accounts they're on the team for') do not transfer. Every Experience Cloud project is two projects: the site, and the external sharing design.",
    ],
    whenToUse: [
      "Customer self-service: case deflection with knowledge articles plus authenticated case tracking. This is the classic, high-ROI first site.",
  ...
  },
  {
    ...
    packaging: "core",
    tagline:
      "The drag-and-drop page editor is how most Salesforce screens get built, with no code required.",
    mentalModel: [
      ...
      "Dynamic Forms takes it further by breaking the old monolithic 'page layout' into individual field sections you place directly on the canvas, with visibility rules per field or section ('show Discount Reason only when Discount > 20%'). The direction of travel is clear: page layouts are the legacy system, App Builder plus Dynamic Forms is the present. Component visibility rules show or hide anything based on record values, device, or user. They replace a surprising amount of what used to require record types or code.",
    ],
    whenToUse: [
